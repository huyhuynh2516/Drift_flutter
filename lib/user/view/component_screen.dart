...
class ComponentScreen extends StatefulWidget {
  ...
  const ComponentScreen({Key? key, required this.db}) : super(key: key);
...
class _ComponentScreenState extends State<ComponentScreen> {
  ...
  final categories = await widget.db.select(widget.db.loaiLinhKiens).get();
  final linhkiens = await widget.db.select(widget.db.linhKiens).get();
  ...
  Future<void> _filterByCategory(int? categoryId) async {
    ...
    if (categoryId == null) {
      list = await widget.db.getAllLinhKien();
    } else {
      list = await widget.db.getLinhKienByCategory(categoryId);
    }
          ...
          content: SingleChildScrollView(
            child: Column(
              children: [
                ...
                // 🔽 Dropdown chọn loại linh kiện (được sửa)
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final newComponent = LinhKiensCompanion.insert(
                  name: nameController.text,
                  image: drift.Value(imageController.text),
                  soluong: drift.Value(int.tryParse(soluongController.text) ?? 0),
                  gia: drift.Value(double.tryParse(giaController.text) ?? 0),
                  categoryId: drift.Value(selectedLoaiId),
                );

                await widget.db.into(widget.db.linhKiens).insert(newComponent);
                ...
                : GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ComponentCard(
                        ...
                        onEdit: () async {
                          final giaController = TextEditingController(text: lk.gia.toString());
                          final soluongController = TextEditingController(text: lk.soluong.toString());
                                ...
                                ElevatedButton(
                                  onPressed: () async {
                                    final updated = lk.copyWith(
                                      ...
                                      soluong: int.tryParse(soluongController.text) ?? lk.soluong,
                                    );
                                    await (widget.db.update(widget.db.linhKiens)
                                    ..where((tbl) => tbl.id.equals(updated.id)))
                                  .write(
                                    LinhKiensCompanion(
                                      name: drift.Value(updated.name),
                                      image: drift.Value(updated.image),
                                      soluong: drift.Value(updated.soluong),
                                      gia: drift.Value(updated.gia),
                        ...
                        onDelete: () async {
                          await widget.db.deleteLinhKien(lk.id!);
                          await _filterByCategory(selectedCategoryId);
                        },
                      );
                    },
